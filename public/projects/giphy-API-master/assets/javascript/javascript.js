$(document).ready(function() {
  ...
  function displayGifs() {
    const action = $(this).attr('data-name');
    const queryURL =
    ...
    $.ajax({
      ...
      const results = response.data; //shows results of gifs
      if (results == '') {
        alert("There isn't a gif for this selected button");
      }
      for (let i = 0; i < results.length; i++) {
        const gifDiv = $('<div>'); //div for the gifs to go inside
        gifDiv.addClass('gifDiv');
        // pulling rating of gif
        const gifRating = $('<p>').text('Rating: ' + results[i].rating);
